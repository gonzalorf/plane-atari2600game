;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Comenzado-Started: 10/05/2001 - 18:00 Hs - Córdoba
;
;Gonzalo Fernández
;
...
;	lda planeEnemyColor+14	;to avoid some cylce counting problems
...
planeDone
	...
	sta COLUP1			;3	27 cycles
sprite1Done_C

;second line
	sta WSYNC			;k5/l2 River - Island - misiles
...
f14     lda #%00000010          ;2
        eor colorBk1            ;3
        sta colorBk1            ;3	17 cycles in the worst case
...
mov	inc moveBk	;5	;	This variable is for the 
				;	Background movement
	lda moveBk	;3
	sta banda	;3	;	This is the variable that makes the movement of the river
	cmp #$10	;2
	bne movDone	;2
	lda #$00  	;2
	sta moveBk	;3
...
joyButton		
	...
	bmi buttonDone
	lda #$01
	sta M0Control		;optimizar usando la misma variable para M0Control y VerPosM0
	lda #10
	sta VerPosM0
buttonDone
...
c1	clc
	lda spritesL,X       ; Init Pointers
	sbc VerPosSprite3
	adc #12
	sta sprite3Ptr
	lda spritesH,X       ; Init Pointers
	sta sprite3Ptr+1

	clc
	lda colorsL,X   ; Init Pointers
	sbc VerPosSprite3
	adc #12
	sta spriteColor3Ptr 
	lda colorsH,X       ; Init Pointers
	sta spriteColor3Ptr+1
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	ORG $FA00
objects		.byte	#%00000000, #%00100001, #%00100001, #%01000001, #%01000001, #%00010010, #%00100010, #%00000011
		.byte	#%00000100, #%00000101, #%00000110, #%00000111, #%00001000, #%00001001, #%00001010, #%00001011	



spritesH	.byte	#>nullSprite, #>planeEnemy, #>jet, #>invisible, #>nullSprite, #>palmera, #>islander1, #>tank1, #>tank2, #>tank3, #>fuelTank, #>blind
spritesL	.byte	#<nullSprite, #<planeEnemy, #<jet, #<invisible, #<nullSprite, #<palmera, #<islander1, #<tank1, #<tank2, #<tank3, #<fuelTank, #<blind
colorsH		.byte	#>palmeraColor, #>planeEnemyColor, #>jetColor, #>palmeraColor, #>palmeraColor, #>palmeraColor, #>islanderColor, #>planeEnemyColor, #>jetColor
colorsL		.byte	#<palmeraColor, #<planeEnemyColor, #<jetColor, #<palmeraColor, #<palmeraColor, #<palmeraColor, #<islanderColor, #<planeEnemyColor, #<jetColor
...
fuelTank	.byte	%01000001		;player1
		.byte	%01000001
		.byte	%01111111
		.byte	%01000001
		.byte	%01111111
...
invisible	.byte	%00111000		;player1
		.byte	%00000000
		.byte	%00000000
		.byte	%00000000
		.byte	%00000000
...
backplaneEnemy	.byte	%00000000		;player1
		.byte	%00011000		
		.byte	%00100100
		.byte	%00000000
		.byte	%00000000
